...
def delete_user_by_id(con,user_id):
    ...
    con.commit()
    print(f"User with [{user_id}] was deleted successfully")
...
def main():
    ...
    if user_input=="1":
        ...
        confirmation=input("Do you want to delete all the data?[y/n]:")
        ...
        if user_id.isnumeric():
            column_name=input(
                f"Enter the column that you want to edit. Please make sure column is within {COLUMNS}:"
            )
